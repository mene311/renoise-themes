...
const insertTheme = db.prepare(`
  INSERT INTO themes (
    ...
    preview_slug, preview_views, preview_error, status,
    ...
    @previewSlug, @previewViews, @previewError, @status,
...
export function saveTheme({ name, filename, originalName, author, description, screenshots, paletteSVG, previewSlug, previewViews, previewError, totalColorEntries, stats, tags, topColors, status = 'draft' }) {
...
const logRequestStmt = db.prepare(`INSERT INTO request_log (path, method, status, referrer) VALUES (?, ?, ?, ?)`);
export function logRequest(path, method, status, referrer) {
  logRequestStmt.run(path, method, status, referrer || null);
}

export function getRecent404s(limit = 50) {
  return db.prepare(`SELECT path, referrer, COUNT(*) as hits, MAX(created_at) as last_seen FROM request_log WHERE status >= 400 GROUP BY path ORDER BY hits DESC LIMIT ?`).all(limit);
}
