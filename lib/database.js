...
const searchThemesByTagQuery = db.prepare(`
  ...
  WHERE (th.name LIKE ? OR th.author LIKE ? OR t.name LIKE ?)
...
export function getUserStats(username) {
  const themes = db.prepare(`SELECT COALESCE(SUM(likes),0) as totalLikes, COALESCE(SUM(downloads),0) as totalDownloads, COUNT(*) as themeCount FROM themes WHERE author = ?`).get(username);
