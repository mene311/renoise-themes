...
const ROLE_RULES = [
  ...
  { pattern: /Body/i,                   role: 'background', weight: 4 },
...
function classifyElement(tagName) {
  // Get just the last part of the path (after last ›)
  ...
  // Default: unknown UI element, low weight
...
export function parseThemeFile(filePath) {
  ...
  for (const { path, value } of leaves) {
    ...
    const { role, weight } = classifyElement(path);
